...
if($account["id"]>0) {
    ...
    if($minutes>0) {
        ...
        if($total_match >= $mix) echo '<div class="success">NỘP BÀI THÀNH CÔNG. Kết quả bài kiểm tra của bạn: <strong>' . $total_match . '/' . $count . '</strong> (câu). Đạt '.$score_student.' %</div>';
        else echo '<div class="failed">NỘP BÀI THÀNH CÔNG. Kết quả bài kiểm tra của bạn: <strong>' . $total_match . '/' . $count . '</strong> (câu).Đạt '.$score_student.' %</div>';
