...
if($account["id"]>0) {
    ...
    if($minutes>0 && $status==0) {
        ...
        foreach($answer as $key => $value) {
            ...
            // Tạo HTML cho kết quả câu hỏi
            $results_html .= '<div class="question">';
            $results_html .= '<label class="question-title">Câu ' . $indexQuestion . ' :</label>';
            $results_html .= '<div class="question-content">' . stripslashes($row['content']) . '</div>';

            if ($test) {
                $results_html .= '<div class="question-answer" style="color:#38b34a;">Đúng!</div>';
            } else {
                $results_html .= '<div class="question-answer" style="color:#e75050;">Sai!</div>';
            }
