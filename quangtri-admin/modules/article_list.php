...
if(isset($_POST['idDel'])){
	...
	$i = 0;
	foreach($rows as $row) {
		$mask = $dir_dest . DS . "article" . DS . $row['folder'];
...
<!-- /.row -->
<?php echo dashboardCoreAdminTwo("article_list;".$category_id_core)?>
<div class="row">
	<div class="col-lg-12">
		<div class="panel panel-default panel-no-border">
			<div class="table-responsive">
				<form action="?<?php echo TTH_PATH?>=article_list" method="post" id="deleteArt">
					<input type="hidden" name="list_id" value="<?php echo $article_menu_id?>" />
							...
							<tr>
								...
								<td align="center">
									<?php echo ($row["img"]=='-no-')?
										...
										<script>
											var image = \'<img src="../uploads/article/' . str_replace(DS, '/', $row['folder']) . $row["img"].'">\';
								...
								<?php
								//-----------Kiểm tra phân quyền ---------------------------------------
								if(in_array("article_edit;".$category_id_core,$corePrivilegeSlug) && in_array("active;".$category_id_core, $corePrivilegeSlug)) {
								?>
								<td align="center">
					...
					<div class="row">
						...
						<div class="col-sm-6" align="right" style="padding: 7px 0;">
							...
							<input type="button" class="btn btn-primary btn-xs <?php echo in_array("article_del;".$category_id_core,$corePrivilegeSlug)? "confirmManager" : "alertManager"?> " value="Xóa" name="deleteArt">
						</div>
					</div>
				</form>
			</div>
