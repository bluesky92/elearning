...
if($del != 0) {
	...
	$rows = $db->select();
	foreach($rows as $row) {
		$mask = $dir_dest . DS . "article_menu" . DS . $row['folder'];
		if(!empty($row['img']) && glob($mask . '*' . $row['img'])) {
			array_map("unlink", glob($mask . '*' . $row['img']));
		}
	}
...
function loadMenuCategory($db, $level, $parent, $category_id){
	...
	foreach($rows2 as $row) {
		...
		<tr>
			...
			if(in_array("article_menu_edit;".$row["category_id"],$corePrivilegeSlug) && in_array("active;".$row["category_id"], $corePrivilegeSlug)) {
				?>
				<td align="right">
			...
			<td align="center">
				<?php echo ($row["img"]=='-no-')?
					...
					<script>
							var image = \'<img src="../uploads/article_menu/' . str_replace(DS, '/', $row['folder']) . $row["img"].'">\';
			...
			<?php
			if ($level < 20){
				loadMenuCategory($db, $level+20, $row["article_menu_id"]+0, $row["category_id"]+0);
			}
			?>
		</tr>
	<?php
