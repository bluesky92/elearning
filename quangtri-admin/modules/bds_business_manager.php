...
<div class="row">
    <ol class="breadcrumb">
		...
		<span style="float: right; font-weight: bold;">
			<a href="?<?php echo TTH_PATH?>=bds_business_add&id=1"><i class="fa fa-plus-square-o"></i> Thêm tin mới</a>
		</span>
    </ol>
</div>
<!-- /.row -->
		    ...
		    <div class="table-responsive">
                <table class="table table-manager table-hover">
                    ...
                    <tr class="category">
                        ...
                        <td align="center">
                            <a href="?<?php echo TTH_PATH?>=bds_business_menu_add&id_cat=<?php echo $row["category_id"]?>"><img data-toggle="tooltip" data-placement="left" title="Thêm mục" src="images/add.png"></a>
                            &nbsp;
...
function loadMenuCategory($db, $level, $parent, $category_id){
	...
	foreach($rows2 as $row) {
	    ...
	    <tr>
			...
			?>
			<td align="center">
				<?php echo showSort("sortpro".$row["bds_business_menu_id"]."", $countList,$row["sort"], $width, 0, $row["bds_business_menu_id"] ,'bds_business_menu', 1);?>
			</td>
			<td align="center">
			    <?php echo ($row["is_active"]+0==0)?
			...
			?>
			<td align="center">
				<?php echo showSort("sortpro".$row["bds_business_menu_id"]."", $countList,$row["sort"], $width, 0, $row["bds_business_menu_id"] ,'bds_business_menu', 0);?>
			</td>
			<td align="center">
				<?php echo ($row["is_active"]+0==0)?
					'<div class="btn-event-close alertManager" data-toggle="tooltip" data-placement="top" title="Mở"></div>'
            ...
            <td align="center">
				...
				if ($level < 30){
	            ?>
                <a href="?<?php echo TTH_PATH?>=bds_business_menu_add&id_cat=<?php echo $row["category_id"]?>&id_pro=<?php echo $row["bds_business_menu_id"]?>"><img data-toggle="tooltip" data-placement="left" title="Thêm mục" src="images/add.png"></a>
                &nbsp;
				<?php } else { ?>
					<span style="width: 16px; height: 1px; display: inline-block;""></span>
					&nbsp;
				<?php } ?>
	            <a href="?<?php echo TTH_PATH?>=bds_business_menu_edit&id=<?php echo $row["bds_business_menu_id"]?>"><img data-toggle="tooltip" data-placement="top" title="Chỉnh sửa" src="images/edit.png"></a>
                &nbsp;
	            <?php
	            ...
	            <?php
		            ...
		            ?>
		            <a class="confirmManager" style="cursor: pointer;" id="?<?php echo TTH_PATH?>=bds_business_manager&del=<?php echo $row["bds_business_menu_id"]?>"><img data-toggle="tooltip" data-placement="right" title="Xóa mục" src="images/remove.png"></a>
	            <?php }
	            //----------- end if ------------
	            ?>
            </td>
	        <td align="center">
		        <a href="?<?php echo TTH_PATH?>=bds_business_list&id=<?php echo $row['bds_business_menu_id']?>"><img data-toggle="tooltip" data-placement="top" title="Danh sách bài viết" src="images/list.png"></a>
	        </td>
	        <?php
			if ($level < 30){
