...
if($typeFunc=='edit'){
	...
	if(empty($name)) $error = '<span class="show-error">Vui lòng nhập tiêu đề.</span>';
		...
		if($OK) {
			...
			$data = array(
				...
				'hot'=>$hot+0,
				'modified_time'=>time(),
				'link2Documents'=>$db->clearText($link2Documents),
	...
	foreach($rows as $row) {
		...
		$hot			    = $row['hot']+0;
		$link2Documents     = $row['link2Documents'];
	}
}
if(!$OK) courses("?".TTH_PATH."=courses_edit", "edit", $courses_id, $product_id, $name, $name_video, $name_document, $document_title, $content, $test, $practice, $is_active, $hot,$link2Documents, $error);