...
function loadMenuCategory($db, $level, $parent, $category_id){
	...
	foreach($rows2 as $row) {
	    ...
	    <tr>
			...
			if(in_array("gallery_menu_edit;".$row["category_id"],$corePrivilegeSlug) && in_array("active;".$row["category_id"], $corePrivilegeSlug)) {
			?>
			<td align="right">
	        ...
	        <?php
			if ($level <= 20){
	            loadMenuCategory($db, $level+20, $row["gallery_menu_id"]+0, $row["category_id"]+0);
			}
	        ?>
	    </tr>
	<?php
