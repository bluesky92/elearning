...
//
function forum($act, $typeFunc, $forum_id, $forum_menu_id, $name, $title, $description, $keywords, $content, $is_active, $hot, $error) {
	global $db, $tth;
	$db->table = "forum_menu";
	...
	<div class="col-lg-12">
		<div class="panel panel-default">
			<div class="panel-heading">
				<i class="fa fa-files-o"></i> Nội dung đề tài
			</div>
				...
				<form action="<?php echo $act?>" method="post" enctype="multipart/form-data">
					...
					<table class="table table-hover">
						...
						<tr>
							...
							<td><?php echo categoryName($forum_menu_id);?></td>
						</tr>
						<tr>
						...
						<tr>
							<td colspan="2" align="center">
								...
								<button type="button" class="btn btn-form-info btn-form" onclick="location.href='?<?php echo TTH_PATH?>=forum_list&id=<?php echo $forum_menu_id?>'">Thoát</button>
							</td>
						</tr>
					</table>
				</form>
...
function categoryName($id) {
	echo '<select name="forum_menu_id" class="form-control">';
	global $db;
	$db->table = "category";
...
function loadMenuCategory($db, $level, $parent, $category_id, $id){
	...
	foreach($rows2 as $row) {
		if ($level <= 3){
			echo "<option value='".$row["forum_menu_id"]."'";
			if ($id==$row["forum_menu_id"]) echo " selected ";
			echo ">".$space.stripslashes($row["name"])."</option>";
				loadMenuCategory($db, $level+2, $row["forum_menu_id"]+0, $row["category_id"]+0, $id);
		}
	}
}
