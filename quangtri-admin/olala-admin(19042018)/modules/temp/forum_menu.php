...
//
function forumMenu($act, $typeFunc, $forum_menu_id, $category_id, $name, $slug, $title, $description, $keywords, $parent, $is_active, $hot, $folder, $img, $comment, $error) {
	...
	<div class="col-lg-12">
		<div class="panel panel-default">
			<div class="panel-heading">
				<i class="fa fa-files-o"></i> Diễn đàn
			</div>
			<div class="panel-body">
				<form action="<?php echo $act?>" method="post" enctype="multipart/form-data">
					<input type="hidden" name="typeFunc" value="<?php echo $typeFunc?>" />
					<input type="hidden" name="parent" value="<?php echo $parent?>" />
					<input type="hidden" name="forum_menu_id" value="<?php echo $forum_menu_id?>" />
					...
					<table class="table table-hover" style="width: 80%;">
						<tr>
							<td width="200px"><label>Tên chuyên mục:</label></td>
						...
						<tr>
							...
							<td class="element-relative">
								...
								<div data-toggle="tooltip" data-placement="top" title="Tạo liên kết tĩnh" class="btn-get-slug" onclick="return getSlug('forum')"></div>
							</td>
						</tr>
						<tr>
						...
						<tr>
							<td colspan="2" align="center">
								...
								<button type="button" class="btn btn-form-info btn-form" onclick="location.href='?<?php echo TTH_PATH?>=forum_manager'">Thoát</button>
							</td>
						</tr>
					</table>
				</form>
...
<script>
	$('.file-img').fileinput({
		<?php if($img!='no' && $img!='') { ?>
		initialPreview: [
			"<img src='../uploads/forum_menu/<?php echo str_replace(DS, '/', $folder) . $img?>' class='file-preview-image' title='<?php echo $img?>' alt='<?php echo $img?>'>"
		],
		<?php } ?>
...
function loadMenuCategory($db, $level, $parent, $category_id, $par){
	...
	foreach($rows2 as $row) {
		if ($level <= 2){
			echo "<option value='".$row["category_id"]."'";
			if ($par==$row["forum_menu_id"]) echo " selected ";
			echo ">".stripslashes($row["name"])."</option>";
				loadMenuCategory($db, $level+2, $row["forum_menu_id"]+0, $row["category_id"]+0, $par);
		}
	}
}
...
function sortAcs($id, $parent){
	global $db;
	$db->table = "forum_menu";
	$db->condition = "`category_id` = $id and `parent` = $parent";
	$db->order = "";
	$db->limit = "";
	$db->select();
	return $db->RowCount;
